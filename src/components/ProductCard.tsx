...
//   alt={product.name}
//   className="h-full w-full object-cover object-center mx-auto transition-transform duration-300 group-hover:scale-105"
// />
  ...
  return (
    <Link to={`/product/${product.id}`} className={`group block ${className}`}>
      ...
      <div className="mt-2">
        ...
        <h3 className="mt-1 text-sm font-medium line-clamp-1">
          {product.name}
        </h3>
        <p className="mt-1 text-sm font-medium">{formatPrice(product.price)}</p>
      </div>
